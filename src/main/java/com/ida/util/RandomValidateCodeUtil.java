...
public class RandomValidateCodeUtil {
    ...
    //randString 随机产生数字与字母组合的字符串
    private String randString = "0123456789";
    //图片宽、高、干扰线数量、  随机产生字符数量
    ...
    public Object[] getRandcode(HttpServletRequest request, HttpServletResponse response) {
        ...
        // 将随机生成的验证码放到session中
        request.getSession().setAttribute("randomCode",randomString);
        String test = (String) request.getSession().getAttribute("randomCode");
        g.dispose();

        return  new Object[]{test,image};
